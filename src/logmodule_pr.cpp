...
struct PlayRequestLogger::Impl {
    ...
    boost::system_time lastRequest;
    boost::system_time lastVocal;
    ...
    /**
      ...
      */
    Impl(PlayRequestLogger &mod) : module(mod), fCallbackMutex(AL::ALMutex::createALMutex()) {
        ...
        try {
            ...
            qiLogError("PlayRequestLogger") << "Error creating proxy to ALMemory" << e.toString() << std::endl;
        }
        // Declare events generated by this module, subscribe to external events
        try {
            memoryProxy->declareEvent("CallChildPR", "PlayRequestLogger");
            memoryProxy->declareEvent("EndSessionPR", "PlayRequestLogger");
            memoryProxy->subscribeToEvent("StartSessionPR", "PlayRequestLogger", "onStartLogger");
            childCount = 0;
            ...
            qiLogError("PlayRequestLogger") << "Error setting up PlayRequestLogger" << e.toString() << std::endl;
        }
    ...
    void startLogger() {
        ...
        filename << "/home/nao/naoqi/modules/logs/" << now.date().year() << "_" << static_cast<int>(now.date().month())
                 << "_" << now.date().day() << "_" <<  now.time_of_day().hours() << now.time_of_day().minutes() << "_PlayRequest.txt";
        outputFileLock.lock();
        ...
        try {
            memoryProxy->subscribeToEvent("FaceDetected", "PlayRequestLogger", "onFaceDetected");
            memoryProxy->subscribeToEvent("ChildCalledPR", "PlayRequestLogger", "onChildCalled");
            memoryProxy->subscribeToEvent("EndSessionPR", "PlayRequestLogger", "onStopLogger");
            memoryProxy->subscribeToEvent("SoundClassified", "PlayRequestLogger", "onSoundClassified");
            classificationProxy->callVoid("pocni_klasifikaciju", parametri);
        }
        catch (const AL::ALError& e) {
            qiLogError("PlayRequestLogger") << "PlayRequestLogger Error subscribing to events" << e.toString() << std::endl;
        }
    ...
    void operator()() {
        ...
        while( true ) {
            ...
            try {
                ...
                if( iteration >= 4 && !ended) {
                    // Log SE - session ended event
                    log("SE", 1);
                    ended = true;
                    // Raise EndSession event
                    memoryProxy->raiseEvent("EndSessionPR", AL::ALValue(1));
                }
                ...
                long long sinceLastRequest = timeDiff.total_milliseconds();

                if( sinceLastRequest >= 5000 && requestFinished && !ended) {
                    // Stop sound classification
                    classificationProxy->callVoid("prekini_klasifikaciju");
                    log("CS", iteration+1);
                    requestFinished = false;
                    memoryProxy->raiseEvent("CallChildPR", AL::ALValue(iteration+1));
                }
...
//                        lastRequest = boost::get_system_time();
//                    }
...
PlayRequestLogger::PlayRequestLogger(boost::shared_ptr<AL::ALBroker> pBroker, const std::string& pName) :  AL::ALModule(pBroker, pName) {
    ...
    BIND_METHOD(PlayRequestLogger::onFaceDetected);

    functionName("onStartLogger", getName(), "Callback for SessionStart event");
    BIND_METHOD(PlayRequestLogger::onStartLogger);

    functionName("onStopLogger", getName(), "Callback for EndSession event");
    BIND_METHOD(PlayRequestLogger::onStopLogger);

    functionName("onChildCalled", getName(), "Callback for ChildCalled event");
    BIND_METHOD(PlayRequestLogger::onChildCalled);

    functionName("onSoundClassified", getName(), "Callback for ChildCalled event");
    BIND_METHOD(PlayRequestLogger::onSoundClassified);
}

PlayRequestLogger::~PlayRequestLogger() {
    //Cleanup code
}

void PlayRequestLogger::init() {
    ...
    try {
        ...
        qiLogError("PlayRequestLogger") << e.what() << std::endl;
    }
    qiLogVerbose("PlayRequestLogger") << "PlayRequestLogger initialized" << std::endl;
}

void PlayRequestLogger::onFaceDetected() {
    ...
    impl->memoryProxy->unsubscribeToEvent("FaceDetected", "PlayRequestLogger");
    // Update the lastFace time
    impl->lastFace = boost::get_system_time();

    // Check validity of the face
    if( face.getSize() < 2 ) {
        qiLogError("PlayRequestLogger") << "Face detected but data is invalid, size " << face.getSize() << std::endl;
    }
    else {
    ...
    impl->memoryProxy->subscribeToEvent("FaceDetected", "PlayRequestLogger", "onFaceDetected");
}

void PlayRequestLogger::onStartLogger() {
    // Thread safety of the callback
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscribe from event, maybe this can be omitted
    impl->memoryProxy->unsubscribeToEvent("StartSessionPR", "PlayRequestLogger");
    ...
    impl->memoryProxy->subscribeToEvent("ChildCalledPR", "PlayRequestLogger", "onChildCalled");
}

void PlayRequestLogger::onStopLogger(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    // Thread safety of the callback
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscriptions
    impl->memoryProxy->unsubscribeToEvent("EndSessionPR", "PlayRequestLogger");

    // Interupt the execution of the scheduler thread
    impl->t->interrupt();
    qiLogWarning("PlayRequestLogger") << "Scheduler thread interrupted\n";
    ...
    try {
        impl->memoryProxy->unsubscribeToEvent("FaceDetected", "PlayRequestLogger");        
        impl->memoryProxy->subscribeToEvent("StartSessionPR", "PlayRequestLogger", "onStartLogger");
        impl->memoryProxy->unsubscribeToEvent("SoundClassified", "PlayRequestLogger");
        impl->memoryProxy->unsubscribeToEvent("ChildCalledPR", "PlayRequestLogger");
        impl->classificationProxy->callVoid("prekini_klasifikaciju");
    }
    catch (const AL::ALError& e) {
        qiLogError("PlayRequestLogger") << "Error managing events" << e.toString() << std::endl;
    }
    qiLogWarning("PlayRequestLogger") << "Ending session, closing log file\n";
    // Close the output file
...
void PlayRequestLogger::onChildCalled(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    // Thread safety of the callback
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscription
    impl->memoryProxy->unsubscribeToEvent("ChildCalledPR", "PlayRequestLogger");
    // Update the time of the last call
    impl->lastRequest = boost::get_system_time();
    ...
    // Call is finished
    impl->requestFinished = true;
    // Subscribe back to the same event
    impl->memoryProxy->subscribeToEvent("ChildCalledPR", "PlayRequestLogger", "onChildCalled");
}

void PlayRequestLogger::onSoundClassified(const std::string &key, const AL::ALValue &value, const AL::ALValue &msg) {
    // Thread safety of the callback
    AL::ALCriticalSection section(impl->fCallbackMutex);
    // Unsubscription
    impl->memoryProxy->unsubscribeToEvent("SoundClassified", "PlayRequestLogger");
    ...
    // Subscribe back to the same event
    impl->memoryProxy->subscribeToEvent("SoundClassified", "PlayRequestLogger", "onSoundClassified");
}
